...
with st.expander("Select the KPIs Reports for each Technology",expanded=True):
    ...
    if btn_initiate_Wcl:
        if not files_2G and not files_3G and not files_4G and not files_5G:
            ...
            if files_2G:
                lataftaf.get_wcl(st.session_state.table_criteria,files_2G,'2G',threshold_days)
            
            if files_3G:
                lataftaf.get_wcl(st.session_state.table_criteria,files_3G,'3G',threshold_days)

            if files_4G:
                lataftaf.get_wcl(st.session_state.table_criteria,files_4G,'4G',threshold_days)

            if files_5G:
                lataftaf.get_wcl(st.session_state.table_criteria,files_5G,'5G',threshold_days)
